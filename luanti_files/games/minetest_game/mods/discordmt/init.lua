...
local timeout = 10
local host = settings:get('discord.host') or 'discord_relay'
local base_url = 'http://' .. tostring(host) .. ':' .. tostring(port)

discord = {}

-- Color used for messages coming from Discord (in-game display)
discord.text_colorization = '#0000ff'
...
-- Drain any messages queued by other mods before `discord` was available
if rawget(_G, "_discord_pending") and type(_G._discord_pending) == "table" then
    for _, msg in ipairs(_G._discord_pending) do
        pcall(function()
            discord.send(msg)
        end)
    end
    _G._discord_pending = nil
end
...
minetest.after(0.5, minetest.register_on_chat_message, function(name, message)
    -- Send player's chat to Discord
